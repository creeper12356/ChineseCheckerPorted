...
public class TouchDo {
    ...
    public static String[] strClass = new String[] { "noClass", "TileView", "MenuView", "StoryView", "MiniGameView",
            "ContinentalView", "BoardView", "ConfigView", "HelpView", "RankView" };
    ...
    public static int getTouchAction(int n, int n2) {
        ...
        n2 += 0;
        for (int i = 0; i < touchNum; ++i) {
            if (n < touchArray[i][0] || n > touchArray[i][0] + touchArray[i][2] || n2 < touchArray[i][1]
                    || n2 > touchArray[i][1] + touchArray[i][3])
                continue;
            n3 = touchArray[i][4];
            break;
        }
